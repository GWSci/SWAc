...
def _do_nitrate_calculations(blackboard):
	...
	if (ff.use_nitrate_intermediate_results):
		write_nitrate_intermediate_results(blackboard)
...
def write_nitrate_intermediate_results(blackboard):
	if (blackboard.node != ff.nitrate_intermediate_results_node_index):
		return

	faux_data = {"params" : {"run_name" : "intermediate_results"}}
	filename = _make_run_filename_with_suffix(faux_data, f"_node_{blackboard.node}_calculation.csv")
	with open(filename, "wb") as f:
		write_heading_row(f)
		f.write(b"\r\n")
		for day in range(len(blackboard.days)):
			write_intermediate_row(f, blackboard, day)
			f.write(b"\r\n")

def write_heading_row(f):
	f.write(b'"a"')
	f.write(b',"ae"')
	f.write(b',"alpha"')
	f.write(b',"cell_area_m_sq"')
	f.write(b',"day"')
	f.write(b',"dSMD_array_mm_per_day"')
	f.write(b',"effective_porosity"')
	f.write(b',"her_array_mm_per_day"')
	f.write(b',"historical_nitrate_reaching_water_table_array_tons_per_day"')
	f.write(b',"infiltration_recharge"')
	f.write(b',"interflow_to_rivers"')
	f.write(b',"interflow_volume"')
	f.write(b',"M_soil_in_kg"')
	f.write(b',"M_soil_tot_kg"')
	f.write(b',"m0_array_kg_per_day"')
	f.write(b',"m1_array_kg_per_day"')
	f.write(b',"m1a_array_kg_per_day"')
	f.write(b',"m2_array_kg_per_day"')
	f.write(b',"m3_array_kg_per_day"')
	f.write(b',"m4_array_kg_per_day"')
	f.write(b',"macropore_att_mm_per_day"')
	f.write(b',"macropore_dir_mm_per_day"')
	f.write(b',"mass_balance_error_kg"')
	f.write(b',"mi_array_kg_per_day"')
	f.write(b',"nitrate_depth_to_water"')
	# f.write(b',"nitrate_loading"')
	f.write(b',"nitrate_reaching_water_table_array_from_this_run_kg_per_day"')
	f.write(b',"nitrate_reaching_water_table_array_from_this_run_tons_per_day"')
	f.write(b',"nitrate_reaching_water_table_array_tons_per_day"')
	f.write(b',"node"')
	f.write(b',"p_non"')
	f.write(b',"p_smd"')
	f.write(b',"perc_through_root_mm_per_day"')
	f.write(b',"Pherperc"')
	f.write(b',"Pro"')
	f.write(b',"proportion_reaching_water_table_array_per_day"')
	f.write(b',"p_non_her"')
	f.write(b',"Psmd"')
	f.write(b',"Psoilperc"')
	f.write(b',"rainfall_ts"')
	f.write(b',"runoff_mm_per_day"')
	f.write(b',"runoff_recharge_mm_per_day"')
	f.write(b',"smd"')
	f.write(b',"TAW_array_mm"')
	f.write(b',"total_NO3_to_receptors_kg"')
	f.write(b',"mu"')
	f.write(b',"sigma"')

def write_intermediate_row(f, blackboard, day):
	f.write(b"%g" % blackboard.a)
	f.write(b",%g" % blackboard.ae[day])
	f.write(b",%g" % blackboard.alpha)
	f.write(b",%g" % blackboard.cell_area_m_sq)
	f.write(b",%g" % day)
	f.write(b",%g" % blackboard.dSMD_array_mm_per_day[day])
	f.write(b",%g" % blackboard.effective_porosity[0])
	f.write(b",%g" % blackboard.her_array_mm_per_day[day])
	f.write(b",%g" % blackboard.historical_nitrate_reaching_water_table_array_tons_per_day[day])
	f.write(b",%g" % blackboard.infiltration_recharge[day])
	f.write(b",%g" % blackboard.interflow_to_rivers[day])
	f.write(b",%g" % blackboard.interflow_volume[day])
	f.write(b",%g" % blackboard.M_soil_in_kg[day])
	f.write(b",%g" % blackboard.M_soil_tot_kg[day])
	f.write(b",%g" % blackboard.m0_array_kg_per_day[day])
	f.write(b",%g" % blackboard.m1_array_kg_per_day[day])
	f.write(b",%g" % blackboard.m1a_array_kg_per_day[day])
	f.write(b",%g" % blackboard.m2_array_kg_per_day[day])
	f.write(b",%g" % blackboard.m3_array_kg_per_day[day])
	f.write(b",%g" % blackboard.m4_array_kg_per_day[day])
	f.write(b",%g" % blackboard.macropore_att_mm_per_day[day])
	f.write(b",%g" % blackboard.macropore_dir_mm_per_day[day])
	f.write(b",%g" % blackboard.mass_balance_error_kg[day])
	f.write(b",%g" % blackboard.mi_array_kg_per_day[day])
	f.write(b",%g" % blackboard.nitrate_depth_to_water[0])
	# f.write(b",%g" % blackboard.nitrate_loading) # TODO
	f.write(b",%g" % blackboard.nitrate_reaching_water_table_array_from_this_run_kg_per_day[day])
	f.write(b",%g" % blackboard.nitrate_reaching_water_table_array_from_this_run_tons_per_day[day])
	f.write(b",%g" % blackboard.nitrate_reaching_water_table_array_tons_per_day[day])
	f.write(b",%g" % blackboard.node)
	f.write(b",%g" % blackboard.p_non[day])
	f.write(b",%g" % blackboard.p_smd[day])
	f.write(b",%g" % blackboard.perc_through_root_mm_per_day[day])
	f.write(b",%g" % blackboard.Pherperc[day])
	f.write(b",%g" % blackboard.Pro[day])
	f.write(b",%g" % blackboard.proportion_reaching_water_table_array_per_day[day])
	f.write(b",%g" % blackboard.p_non_her[day])
	f.write(b",%g" % blackboard.Psmd[day])
	f.write(b",%g" % blackboard.Psoilperc[day])
	f.write(b",%g" % blackboard.rainfall_ts[day])
	f.write(b",%g" % blackboard.runoff_mm_per_day[day])
	f.write(b",%g" % blackboard.runoff_recharge_mm_per_day[day])
	f.write(b",%g" % blackboard.smd[day])
	f.write(b",%g" % blackboard.TAW_array_mm[day])
	f.write(b",%g" % blackboard.total_NO3_to_receptors_kg[day])
	f.write(b",%g" % blackboard.μ[0])
	f.write(b",%g" % blackboard.σ)
