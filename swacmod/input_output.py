...
###############################################################################
def dump_sfr_outputold(data, runoff):
    ...
    from swacmod.model import get_sfr_flows
    ...
    for per in xrange(nper):
        ...
        ro, flow = get_sfr_flows(sorted_by_ca, idx, runoff, done, areas, swac_seg_dic, ro, flow, nodes, per)
        print 'copying'
        for iseg in xrange(nss):
        ...
        #             # not done
        #             if done[node_swac] < 1:
        #                 sd[iseg - 1]['runoff'] += runoff[(nodes * per) + node_swac] * areas[node_swac]
        #                 sd[iseg - 1]['flow'] += acc
        #                 done[node_swac] += 1
        #                 acc = 0.0
