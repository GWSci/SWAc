...
def _get_sfr_file_mf6(data, runoff):
    ...
    nodes = extract_node_count(data)
    # nss
    connectiondata = make_connectiondata(data, sorted_by_ca, nss, seg_swac_dic, swac_seg_dic)
    packagedata = make_packagedata(data, sorted_by_ca, connectiondata)
    perioddata = make_perioddata(data, sorted_by_ca, nss, runoff, swac_seg_dic)
...
def make_perioddata(data, sorted_by_ca, nss, runoff, swac_seg_dic):
    perioddata = {}
    perioddata[0] = []
    str_count = 0
    for node_swac, line in sorted_by_ca.items():
...
def make_connectiondata(data, sorted_by_ca, nss, seg_swac_dic, swac_seg_dic):
    nodes = extract_node_count(data)
