...
class HistoricalNitrateBlackboard():
	def __init__(self):
		...
		self.historical_solute_days = None
		...
		self.truncated_historical_solute_days = None
		self.μ = None
		self.σ = None
	...
	def initialise_blackboard(self, data, output, node):
		...
		self.historical_solute_days = data["series"]["historical_solute_days"]
		self.historical_time_periods = data["params"]["historical_time_periods"]
		self.solute_depth_to_water = data["params"]["solute_depth_to_water"][node]
		self.node = node
...
def calculate_total_days_count_upper_bound(data):
	...
	historical_solute_days_len = 0
	if "historical_solute_days" in data["series"]:
		historical_solute_days_len = len(data["series"]["historical_solute_days"])
	return date_len + historical_solute_days_len
...
def _calculate_historical_solute(blackboard):
	blackboard.historical_mi_array_kg_per_day = _calculate_aggregate_mi_unpacking(blackboard)
	blackboard.truncated_historical_solute_days = _calculate_truncated_historical_solute_days(blackboard)
...
def _calculate_truncated_historical_solute_days(blackboard):
	historical_solute_days = blackboard.historical_solute_days
	days = blackboard.days
	if len(days) == 0:
		return historical_solute_days
	first_new_day = days[0]
	truncated_historical_solute_days = [d for d in historical_solute_days if d < first_new_day]
	return truncated_historical_solute_days

def _calculate_truncated_historical_mi_array_kg_per_day(blackboard):
	truncated_length = len(blackboard.truncated_historical_solute_days)
