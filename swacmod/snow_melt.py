...
class SnowMelt:
	def SnowMelt(self, Date, precip_mm, Tmax_C, Tmin_C, lat_deg, slope, aspect, tempHt, windHt, groundAlbedo, SurfEmissiv, windSp, forest, startingSnowDepth_m, startingSnowDensity_kg_m3):
		## Constants :
		WaterDens = 1000.0            # kg/m3
		lambdaV = 2500.0              # (kJ/kg) latent heat of vaporization
		...
		NewSnowDensity[NewSnowDensity < 50] = 50.0
		NewSnowWatEq = np.copy(precip_m)  # m
		NewSnowWatEq[Tav >= 0] = 0.0        # No new snow if average temp above or equals 0 C
		NewSnow = NewSnowWatEq*WaterDens/NewSnowDensity     # m
		
		jday_string_to_day_in_year = lambda x: float(datetime.strptime(x, "%Y-%m-%d").timetuple().tm_yday)
		...
		SnowDensity 	= np.full_like(precip_mm, 450.0, dtype=float)			#  (kg/m3)  Max density is 450
		SnowDepth       = np.full_like(precip_mm, 0.0, dtype=float)		#  (m)
...
def EstCloudiness(Tx, Tn):
	...
	cl = 1.0 - (trans-transMin) / (transMax-transMin)
	cl[cl > 1] = 1.0
...
def transmissivity(Tx, Tn):
	...
	avDeltaT = np.zeros(Len, dtype=float)
	...
	return(A*(1.0-np.exp(-B*np.power(dT, C))))
