...
class Test_Historical_Solute(unittest.TestCase):
	def test_historical_solute_returns_zeros_when_disabled(self):
		...
		self.assert_historical_solute(expected, input_process_enabled, input_days, input_historical_days, input_historical_mi, input_node)

	def assert_historical_solute(self, expected, input_process_enabled, input_days, input_historical_days, input_historical_mi, input_node):
		data = {
			"params": {
		...
		output = {}
		node = input_node
		process_result = historical_solute.get_historical_solute(data, output, node)
		actual = process_result["historical_solute_reaching_water_table_array_tons_per_day"]
		np.testing.assert_array_almost_equal(expected, actual)

	def test_calculate_truncated_historical_solute_days_when_both_sets_of_date_are_empty(self):
		self.assert_calculate_truncated_historical_solute_days([], [], [])

	def assert_calculate_truncated_historical_solute_days(self, expected, input_historical_solute_days, input_days):
		blackboard = historical_solute.HistoricalSoluteBlackboard()
		blackboard.historical_solute_days = input_historical_solute_days
		blackboard.days = input_days

		actual = historical_solute._calculate_truncated_historical_solute_days(blackboard)

		self.assertEqual(expected, actual)
		
	def test_calculate_truncated_historical_solute_dates_when_historical_dates_are_empty(self):
		input_historical_solute_days = []
		input_days = [date(2023, 1, 3), date(2023, 1, 4), ]
		expected = []
		self.assert_calculate_truncated_historical_solute_days(expected, input_historical_solute_days, input_days)

	def test_calculate_truncated_historical_solute_dates_when_new_dates_are_empty(self):
		input_historical_solute_days = [date(2023, 1, 1), date(2023, 1, 2), ]
		input_days = []
		expected = [date(2023, 1, 1), date(2023, 1, 2), ]
		self.assert_calculate_truncated_historical_solute_days(expected, input_historical_solute_days, input_days)

	def test_calculate_truncated_historical_solute_dates_when_historical_dates_are_contiguous_with_new_dates(self):
		input_historical_solute_days = [date(2023, 1, 1), date(2023, 1, 2), ]
		input_days = [date(2023, 1, 3), date(2023, 1, 4), ]
		expected = [date(2023, 1, 1), date(2023, 1, 2), ]
		self.assert_calculate_truncated_historical_solute_days(expected, input_historical_solute_days, input_days)

	def test_calculate_truncated_historical_solute_dates_when_there_is_a_gap_between_historical_and_new_dates(self):
		input_historical_solute_days = [date(2023, 1, 1), date(2023, 1, 2), ]
		input_days = [date(2023, 1, 7), date(2023, 1, 8), ]
		expected = [date(2023, 1, 1), date(2023, 1, 2), ]
		self.assert_calculate_truncated_historical_solute_days(expected, input_historical_solute_days, input_days)

	def test_calculate_truncated_historical_solute_dates_when_there_is_an_overlap_between_historical_and_new_dates(self):
		input_historical_solute_days = [date(2023, 1, 1), date(2023, 1, 2), date(2023, 1, 3), date(2023, 1, 4), ]
		input_days = [date(2023, 1, 3), date(2023, 1, 4),  date(2023, 1, 5),  date(2023, 1, 6), ]
		expected = [date(2023, 1, 1), date(2023, 1, 2), ]
		self.assert_calculate_truncated_historical_solute_days(expected, input_historical_solute_days, input_days)

	def test_calculate_historical_solute_populates_truncated_historical_solute_dates(self):
		actual = historical_solute._calculate_historical_solute(self.make_sample_blackboard())
		self.assertIsNotNone(actual.truncated_historical_solute_days)

	def test_calculate_historical_solute_populates_truncated_historical_mi_array_kg_per_day(self):
		actual = historical_solute._calculate_historical_solute(self.make_sample_blackboard())
		self.assertIsNotNone(actual.truncated_historical_mi_array_kg_per_day)

	def test_calculate_historical_solute_populates_historic_proportion_reaching_water_table_array_per_day(self):
		actual = historical_solute._calculate_historical_solute(self.make_sample_blackboard())
		self.assertIsNotNone(actual.historic_proportion_reaching_water_table_array_per_day)

	def test_calculate_historical_solute_populates_historical_mass_reaching_water_table_array_kg_per_day(self):
		actual = historical_solute._calculate_historical_solute(self.make_sample_blackboard())
		self.assertIsNotNone(actual.historical_mass_reaching_water_table_array_kg_per_day)

	def test_calculate_historical_solute_populates_historical_solute_reaching_water_table_array_tons_per_day(self):
		actual = historical_solute._calculate_historical_solute(self.make_sample_blackboard())
	...
	def make_sample_blackboard(self):
		...
		blackboard.historical_solute_days = [date(2023, 1, 1), date(2023, 1, 2), ]
	...
	def test_calculate_truncated_historical_mi_array_kg_per_day_does_not_truncate_when_dates_are_longer_than_mi(self):
		expected = [10.0, 20.0]
		truncated_historical_solute_days = [date(2023, 1, 1), date(2023, 1, 2), date(2023, 1, 3), ]
		historical_mi_array_kg_per_day = [10.0, 20.0]
		self.assert_calculate_truncated_historical_mi_array_kg_per_day(
			expected,
			truncated_historical_solute_days,
	...
	def assert_calculate_truncated_historical_mi_array_kg_per_day(
			self,
			expected,
			input_truncated_historical_solute_days,
			input_historical_mi_array_kg_per_day
	):
		blackboard = historical_solute.HistoricalSoluteBlackboard()
		blackboard.truncated_historical_solute_days = input_truncated_historical_solute_days
		blackboard.historical_mi_array_kg_per_day = np.array(input_historical_mi_array_kg_per_day)

		actual = historical_solute._calculate_truncated_historical_mi_array_kg_per_day(blackboard)
	...
	def test__calculate_historic_proportion_reaching_water_table_array_per_day(self):
		...
		blackboard.truncated_historical_solute_days = historic_days
		actual = historical_solute._calculate_historic_proportion_reaching_water_table_array_per_day(blackboard)
	...
	def test_calculate_total_days_count_upper_bound_when_both_days_are_present(self):
		data = {
			"series" : {
				"date": [1, 2],
				"historical_solute_days" : [1, 2, 3],
			},
		}
		actual = historical_solute.calculate_total_days_count_upper_bound(data)
	...
	def test_calculate_total_days_count_upper_bound_when_historical_solute_days_is_missing(self):
		data = {
			"series" : {
				"date": [1, 2],
			},
	...
	def make_reference_proportion_reaching_water_table_for_combined_historic_and_new_periods(self, combined_days):
		solute_blackboard = solute.SoluteBlackboard()
		self.assign_common_blackboard_inputs_for_proportion_reaching_water_table(solute_blackboard)
		solute_blackboard.days = combined_days
		cumulative_proportion_for_the_entire_period = solute._calculate_proportion_reaching_water_table_array_per_day(solute_blackboard)
	...
	def test_calculate_historical_mass_reaching_water_table_array_kg_per_day(self):
		...
		actual = historical_solute._calculate_historical_mass_reaching_water_table_array_kg_per_day(blackboard)
	...
	def test_convert_kg_to_tons_array(self):
		...
		actual = historical_solute._convert_kg_to_tons_array(blackboard)

		expected = np.array([1.0, 2.0, 3.0])
		np.testing.assert_allclose(expected, actual)

	def test_historical_solute_initialise_blackboard(self):
		...
		self.assertEqual(expected.historical_solute_days, actual.historical_solute_days)
	...
	def make_sample_data_output_node(self):
		data = {
			...
			"series": {
				...
				"historical_solute_days" : [date(2023, 1, 1), date(2023, 1, 2)],
			},
		}
		output = {}
		node = 7
	...
	def test_get_historical_solute_return_answer_when_enabled(self):
		data, output, node = self.make_sample_data_output_node()
		actual_output = historical_solute.get_historical_solute(data, output, node)
		actual = actual_output["historical_solute_reaching_water_table_array_tons_per_day"]

		expected_blackboard = historical_solute._calculate_historical_solute(self.make_sample_blackboard())
