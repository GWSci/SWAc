...
def assert_csv_equal(test_case, expected, actual):
	...
	if (len(expected_grid) > 0):
		for col_index in range(len(expected_grid[0])):
			message = f"Difference in row=0, col={col_index}. Expected: {expected_grid[0][col_index]} Actual: {actual_grid[0][col_index]}"
			test_case.assertEqual(expected_grid[0][col_index], actual_grid[0][col_index], message)
